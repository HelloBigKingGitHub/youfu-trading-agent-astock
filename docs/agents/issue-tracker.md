...
Issues and PRDs for this repo live as GitHub issues in `HelloBigKingGitHub/youfu-trading-agent-astock`. Use the `gh` CLI for all operations.
...
## Related but separate: `issues/` post-mortem archive

The repo's top-level `issues/` folder holds hand-written post-mortem records (root-cause analysis of already-fixed problems). It is **not** the live tracker. Skills read from and write to GitHub Issues, not `issues/`.
